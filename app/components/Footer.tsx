...
const SOCIALS = [
  ...
  },
  {
    name: 'TikTok',
    href: '#',
    icon: <svg viewBox="0 0 24 24" className="w-4 h-4" fill="currentColor"><path d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.15 1.07-.14 1.61.24 1.64 1.82 3.02 3.5 2.87 1.12-.01 2.19-.66 2.77-1.61.19-.33.4-.67.41-1.06.1-1.79.06-3.57.07-5.36.01-4.03-.01-8.05.02-12.07z"/></svg>,
  },
];
...
export default function Footer() {
  return (
    <footer className="bg-black border-t border-white/[0.05]">
      <hr className="gold-rule opacity-40" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-[2fr_1fr_1fr] gap-12">
          ...
          >
            <a href="#hero" className="flex items-center gap-3 group mb-5">
              <div className="relative w-9 h-9 rounded-full overflow-hidden opacity-85 group-hover:opacity-100 transition-opacity">
                <Image src="/Logo 2.PNG" alt="Malachias" fill className="object-cover scale-125" />
              </div>
              <span className="font-display text-xl tracking-[0.18em] text-white group-hover:text-[#c9a84c] transition-colors">
                MALACHIAS
              </span>
            </a>
            <p className="label-xs mb-4" style={{ color: 'var(--text-3)', opacity: 1 }}>
              Christian Rock · Veteran Spirit · Faith on Fire
            </p>
            <p className="text-[0.82rem] text-[#4a4438] leading-relaxed max-w-xs mb-8">
            ...
            <div className="flex gap-3">
              {SOCIALS.map(s => (
                <a
                  ...
                  href={s.href}
                  aria-label={s.name}
                  className="w-8 h-8 border border-white/[0.07] flex items-center justify-center text-[#4a4438] hover:text-[#c9a84c] hover:border-[#c9a84c]/30 transition-all duration-300"
                >
                  {s.icon}
                </a>
              ))}
          ...
          >
            <p className="label-xs mb-5" style={{ color: 'var(--text-2)', opacity: 1 }}>Navigate</p>
            <ul className="space-y-3">
              {LINKS.map(l => (
                <li key={l.href}>
                  <a href={l.href} className="text-[0.82rem] text-[#4a4438] hover:text-[#c9a84c] transition-colors duration-300 flex items-center gap-2 group">
          ...
          >
            <p className="label-xs mb-5" style={{ color: 'var(--text-2)', opacity: 1 }}>Contact</p>
            <div className="space-y-4 text-[0.82rem]">
              {[
                ...
                <div key={c.role}>
                  <div className="label-xs mb-1" style={{ color: 'var(--text-3)', opacity: 1 }}>{c.role}</div>
                  <a href={`mailto:${c.email}`} className="text-[#4a4438] hover:text-[#c9a84c] transition-colors duration-300">
                    {c.email}
                  </a>
                </div>
              ))}
              <div className="pt-2 border-t border-white/[0.04]">
                <div className="label-xs mb-1" style={{ color: 'var(--text-3)', opacity: 1 }}>Based In</div>
                <p className="text-[#4a4438]">United States of America</p>
              </div>
            </div>
          </motion.div>
      ...
      <div className="border-t border-white/[0.04]">
        <div className="max-w-6xl mx-auto px-6 py-5 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-[0.68rem] tracking-wider text-[#2a2420]">
            © {new Date().getFullYear()} Malachias. All rights reserved.
          </p>
          <p className="text-[0.68rem] tracking-wider text-[#2a2420]">
            Est. in Faith · Forged in Service · One Nation Under God
          </p>
          <div className="flex gap-4 text-[0.68rem] tracking-wider text-[#2a2420]">
            <a href="#" className="hover:text-[#4a4438] transition-colors">Privacy</a>
            <a href="#" className="hover:text-[#4a4438] transition-colors">Terms</a>
          </div>
        </div>
      </div>
    </footer>
